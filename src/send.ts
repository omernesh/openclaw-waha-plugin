...
// Phase 54-02: Mimicry enforcement — time gate + cap + jitter + typing simulation. DO NOT REMOVE.
...
export async function sendWahaText(params: {
  ...
  // Runs after mute/policy checks and before the WAHA API call.
  // bypassPolicy=true skips all enforcement (/shutup, /join, /leave system commands).
  if (!params.bypassPolicy) {
  ...
  const result = await client.post("/api/sendText", {
    chatId,
    text: textToSend,
    session: client.session,
  ...
  // Called AFTER WAHA API success — failed sends do not consume cap quota.
...
export async function sendWahaMediaBatch(params: {
  ...
  // Phase 54: Batch mimicry pre-check — counts entire batch against hourly cap. DO NOT CHANGE.
...
export async function sendWahaImage(params: {
  ...
  replyToId?: string;
  accountId?: string;
  bypassPolicy?: boolean; // Skip mimicry enforcement (used by system commands like /shutup)
}) {
  const client = getClient(params.cfg, params.accountId);
  assertCanSend(client.session, params.cfg);
  ...
  // Phase 6: Rules-based outbound policy enforcement. DO NOT CHANGE.
  if (!params.bypassPolicy) assertPolicyCanSend(chatId, params.cfg);
  // Phase 54: Mimicry enforcement — time gate + cap + jitter + typing simulation. DO NOT CHANGE.
  if (!params.bypassPolicy) {
    await enforceMimicry({
      session: client.session,
      ...
      accountId: client.accountId,
      cfg: params.cfg,
      messageLength: params.caption?.length ?? 0,
    });
  }
  const filePayload = buildFilePayload(params.file);
  const result = await client.post("/api/sendImage", {
    chatId,
    file: filePayload,
    session: client.session,
...
export async function sendWahaVideo(params: {
  ...
  const filePayload = buildFilePayload(params.file);
  const result = await client.post("/api/sendVideo", {
    chatId,
    file: filePayload,
    session: client.session,
    convert: true,
...
export async function sendWahaFile(params: {
  ...
  const filePayload = buildFilePayload(params.file);
  const result = await client.post("/api/sendFile", {
    chatId,
    file: filePayload,
    session: client.session,
    ...(params.caption ? { caption: params.caption } : {}),
...
export async function sendWahaLinkPreview(params: {
  ...
  const client = getClient(params.cfg, params.accountId);
  assertCanSend(client.session, params.cfg);
  // Phase 54: Mimicry enforcement — link preview cards are new agent content. DO NOT CHANGE.
  await enforceMimicry({
    session: client.session,
    chatId: params.chatId,
    accountId: client.accountId,
  ...
  const result = await client.post("/api/send/link-custom-preview", {
    chatId: params.chatId, session: client.session,
...
export async function sendWahaTextStatus(params: { cfg: CoreConfig; text: string; backgroundColor?: string; font?: number; accountId?: string }) {
  const client = getClient(params.cfg, params.accountId);
  assertCanSend(client.session, params.cfg);
  // Phase 54: Status sends honour time gate but skip hourly cap (isStatusSend=true). DO NOT CHANGE.
  await enforceMimicry({
    session: client.session,
